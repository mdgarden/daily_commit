...
- 스플릿 플레이스멘트 그룹
    - 인스턴스를 다른 하드웨어에 배치하여 가용성으 높이는 서비스
...
- Dyanamo DB
    - 키/밸류 형의 DB가 필요하다 : Dyanamo DB
    - 마이크로 단위의 레이턴시 : Dyanamo DB Accelerator
    - DB에 영향을 주고 싶지 않다 : Dyanamo DB Accelerator
